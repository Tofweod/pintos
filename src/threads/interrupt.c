...
static bool emerge_on_return;    /* Should we emerge threads on interrupt return? */
...
void
intr_emerge_on_return(void)
{
  ASSERT(intr_context());
  emerge_on_return = true;
}

  ...
  if (external) 
    {
      ...
      if (yield_on_return || emerge_on_return) 
      {
        emerge_on_return = false;
        thread_yield();
      }
